...
final class MarkdownQuoteCell: UICollectionViewCell {
    ...
    static func inset(quoteLevel: Int) -> UIEdgeInsets {
        UIEdgeInsets(
            top: 0,
            left: (MarkdownQuoteCell.borderWidth + Sizes.columnSpacing) * CGFloat(quoteLevel),
            bottom: Sizes.rowSpacing,
            right: 0)
    }
    ...
    override func layoutSubviews() {
        ...
        for (i, border) in borders.enumerated() {
            border.frame = CGRect(
                x: (MarkdownQuoteCell.borderWidth + Sizes.columnSpacing) * CGFloat(i),
                y: 0,
                width: MarkdownQuoteCell.borderWidth,
                height: contentView.bounds.height - Sizes.rowSpacing
            )
        }
    }
    ...
    func configure(with model: MarkdownQuoteModel) {
        ...
        for _ in 0 ..< model.level {
            let border = UIView()
            border.backgroundColor = Colors.muted.color
